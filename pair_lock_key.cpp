...
void PairLockKey::compute(int eflag, int vflag) {
  ...
  double rsq, r, std_dev2, lock, factor_lj;
  ...
  for (ii = 0; ii < inum; ii++) {
	...
	std_dev2 = std_dev[itype][jtype] * std_dev[itype][jtype];
        ...
        lock = -amp[itype][jtype] * exp(-(r * r)/(2 * std_dev2) );
	fpair = (r/std_dev2) * lock;
	...
	if (eflag) evdwl = factor_lj * lock - offset[itype][jtype];
...
{
  if (narg != 1) error->all(FLERR, "Pair style lock/key requires exactly one argument: global cutoff");
...
void PairLockKey::coeff(int narg, char **arg)
{
 ...
 // Error checks for invalid parameter values
  if (std_dev_i <= 0 || std_dev_j <= 0) error->all(FLERR, "Standard deviation for lock/key must be positive");
...
double PairLockKey::single(int /*i*/, int /*j*/, int itype, int jtype, double rsq,
                             double /*factor_coul*/, double factor_lj, double &fforce)
{
  double r, std_dev2, lock, fpair;
...
//{
//  // Request a neighbor list, example with some standard options
//  int irequest = neighbor->request(this, instance_me);
//  neighbor->requests[irequest]->id = 1;
//  neighbor->requests[irequest]->half = 1;
//  neighbor->requests[irequest]->newton_pair = newton_pair;
//}
...
void PairLockKey::write_data(FILE *fp)
{
  for (int i = 1; i <= atom->ntypes; i++)
    for (int j = i; j <= atom->ntypes; j++)
...
void PairLockKey::write_data_all(FILE *fp)
{
  for (int i = 1; i <= atom->ntypes; i++)
    for (int j = i; j <= atom->ntypes; j++)
...
void *PairLockKey::extract(const char *str, int &dim)
{
  dim =2;
