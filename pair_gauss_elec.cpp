...
PairGaussElec::PairGaussElec(LAMMPS *lmp) : Pair(lmp), cut(nullptr), offset(nullptr)
{
    single_enable = 1;
    respa_enable = 0; 
    writedata = 1;
...
void PairGaussElec::compute(int eflag, int vflag) {
  int ii, i, j, jj, inum, jnum, itype, jtype;
  double xtmp, ytmp, ztmp, delx, dely, delz, evdwl, fpair;
  double rsq, r, factor_lj, std_dev2, gauss ;
  ...
  for (ii = 0; ii < inum; ii++) {
	...
	if (eflag) evdwl = factor_lj * gauss - offset[itype][jtype];
...
void PairGaussElec::settings(int narg, char **arg)
{
  if (narg != 1) error->all(FLERR, "Pair style gauss/elec must have exactly one argument");
...
void PairGaussElec::coeff(int narg, char **arg)
{
  ...
  double amp_i     = utils::numeric(FLERR, arg[2], false, lmp);
  double std_dev_i = utils::numeric(FLERR, arg[3], false, lmp);
  double amp_j     = utils::numeric(FLERR, arg[4], false, lmp);
  double std_dev_j = utils::numeric(FLERR, arg[5], false, lmp);
  double cut_one   = utils::numeric(FLERR, arg[6], false, lmp);

 // Error checks for invalid parameter values
  if (std_dev_i <= 0 || std_dev_j <= 0) error->all(FLERR, "Standard deviation for gauss/elec must be positive");
...
double PairGaussElec::init_one(int i, int j)
{
...
double PairGaussElec::single(int /*i*/, int /*j*/, int itype, int jtype, double rsq,
                             double /*factor_coul*/, double factor_lj, double &fforce)
{
  double r, gauss, std_dev2, fpair;
...
void PairGaussElec::write_restart(FILE *fp)
{
...
void PairGaussElec::write_restart_settings(FILE *fp)
{
...
void PairGaussElec::read_restart(FILE *fp)
{
...
void PairGaussElec::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
void PairGaussElec::write_data(FILE *fp)
{
  for (int i = 1; i <= atom->ntypes; i++)
    for (int j = i; j <= atom->ntypes; j++)
      fprintf(fp, "%d %d %g %g %g\n", i, j, std_dev[i][j], cut[i][i], amp[i][i]);
}
...
void PairGaussElec::write_data_all(FILE *fp)
{
  for (int i = 1; i <= atom->ntypes; i++)
    for (int j = i; j <= atom->ntypes; j++)
      fprintf(fp, "%d %d %g %g %g\n", i, j, std_dev[i][j], cut[i][j], amp[i][i]);
}


/* ---------------------------------------------------------------------- */

void *PairGaussElec::extract(const char *str, int &dim)
{
  dim =2;
