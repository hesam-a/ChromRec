...
typedef struct
{
    ...
    double Ie_a; /* Informational energy for state a */
    double Ie_b; /* Informational energy for state b */
...
double calculate_stddev(const std::vector<double>& samples)
{
    if (samples.size() < 2) return 0.0;  // Standard deviation is undefined for 1 or fewer samples
    ...
    variance /= (samples.size() - 1);  // Sample variance

    return std::sqrt(variance);
}


// Function to calculate bandwidth using Silverman's rule
double calculate_bandwidth_silverman(const std::vector<double>& samples)
{
    double sigma = calculate_stddev(samples);
    printf("stddev: %9.5f\n", sigma);
...
// Function to calculate the KDE-based density for a single sample
double calc_kde_density(double sample, const std::vector<double>& samples, double bandwidth)
{
    double density = 0.0;
    const double inv_sqrt_2pi = 1.0 / std::sqrt(2 * M_PI);

    for (double neighbor : samples)
    {
        double u = (sample - neighbor) / bandwidth;
        density += inv_sqrt_2pi * std::exp(-0.5 * u * u);
    }

    density /= (samples.size() * bandwidth);
    return density;
}


/* calculate informational energy */
double calc_informational_energy(const std::vector<double>& samples, double bandwidth)
{
    double Ie = 0.0;

    for (double sample : samples)
    {
        double density = calc_kde_density(sample, samples, bandwidth);
        Ie += density * density;
    }
...
static double calc_bar_lowlevel(sample_coll_t* ca, sample_coll_t* cb, double temp, double tol, int type)
{
    ...
    std::vector<double> samples_a,  samples_b;
    std::vector<double> Ie_a_array, Ie_b_array;
    double bandwidth;
    ...
    int counter = 0;
    // Convert state `a` samples to vector and calculate IE using KDE

    for (int i = 0; i < ca->nsamples; i++)
    {
        samples_t* s = ca->s[i];
        sample_range_t* r = &(ca->r[i]);
    
        if (r->use && s->du) // Ensure sample range is usable and `du` exists
        {
            for (int j = r->start; j < r->end; j++)
            {
                samples_a.push_back(s->du[j]); // Capture each individual sample
	        counter += 1;
            }
        }
    }
...
//    {
//        samples_t* s = ca->s[i];
//        if (s && s->du) // Check if s and s->du are not null
//        {
//            samples_a.push_back(*(s->du));  // Assuming `du` points to the energy difference
//            counter += 1;
//            printf("ca->s[i]->du a: %9.5f\n", *(s->du));
//        }
//    }
    printf(" counter: %d \n",counter);

    bandwidth = calculate_bandwidth_silverman(samples_a);
    //printf("bandwith of a: %9.5f\n", bandwidth);

    // Calculate individual IE values for state `a`
    for (double sample : samples_a)
    {
        double density = calc_kde_density(sample, samples_a, bandwidth);
        Ie_a_array.push_back(density * density);  // Store individual IE for each sample
    }

    for (int i = 0; i < cb->nsamples; i++)
    {
        samples_t* s = cb->s[i];
        ...
        if (r->use && s->du) // Ensure sample range is usable and `du` exists
        {
            for (int j = r->start; j < r->end; j++)
            {
                samples_b.push_back(s->du[j]); // Capture each individual sample
            }
        }
    }    
...
//        if (s && s->du) // Check if s and s->du are not null
//        {
//            samples_b.push_back(*(s->du));  // Assuming `du` points to the energy difference
//            printf("cb->s[i]->du b: %9.5f\n", *(s->du));
//        }
//    }

    bandwidth = calculate_bandwidth_silverman(samples_b);
    //printf("bandwith of b: %9.5f\n", bandwidth);

    // Calculate individual IE values for state `b`    
    for (double sample : samples_b)
    {
        double density = calc_kde_density(sample, samples_b, bandwidth);
        Ie_b_array.push_back(density * density);
    }

    double max_Ie_a = *std::max_element(Ie_a_array.begin(), Ie_a_array.end());
    double max_Ie_b = *std::max_element(Ie_b_array.begin(), Ie_b_array.end());
    
    // Normalize IE values for state `a`
    for (auto& Ie_val : Ie_a_array)
    {
        Ie_val /= max_Ie_a;
    }
    
    // Normalize IE values for state `b`
    for (auto& Ie_val : Ie_b_array)
    {
        Ie_val /= max_Ie_b;
    }
    ...
    while (DG2 - DG0 > 2 * tol)
    {
        ...
        for (i = 0; i < ca->nsamples; i++)
        {
            ...
            if (r->use)
            {
                dDG1 += calc_bar_sum(r->end - r->start, s->du + r->start, Wfac1, (M - DG1)) * Ie_a_array[i]; 
    		//printf("Ie_a: %9.5f\n", Ie_a_array[i]);
            }
        }
        for (i = 0; i < cb->nsamples; i++)
        {
            ...
            if (r->use)
            {
                dDG1 -= calc_bar_sum(r->end - r->start, s->du + r->start, Wfac2, -(M - DG1)) * Ie_b_array[i];
    		//printf("Ie_b: %9.5f\n", Ie_b_array[i]);
            }
        }
...
int gmx_abar(int argc, char* argv[])
{
