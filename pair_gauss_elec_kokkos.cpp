...
#include "pair_gauss_elec_kokkos.h"
...
PairGaussElecKokkos<DeviceType>::PairGaussElecKokkos(LAMMPS *lmp) : PairGaussElec(lmp) {
...
/* ---------------------------------------------------------------------- */
template<class DeviceType>
PairGaussElecKokkos<DeviceType>::~PairGaussElecKokkos() {
...
/* ---------------------------------------------------------------------- */
template<class DeviceType>
void PairGaussElecKokkos<DeviceType>::settings(int narg, char **arg) {
  if (narg > 2) error->all(FLERR,"Illegal pair_style command");

  PairGaussElec::settings(1,arg);
}

/* ---------------------------------------------------------------------- */
template<class DeviceType>
void PairGaussElecKokkos<DeviceType>::init_style()

{
  // call the base class, which does neighbor->add_request(this,...)
  PairGaussElec::init_style();
...
/* ---------------------------------------------------------------------- */
template<class DeviceType>
double PairGaussElecKokkos<DeviceType>::init_one(int i, int j) {
  double cutone = PairGaussElec::init_one(i, j);
...
/* ---------------------------------------------------------------------- */
template<class DeviceType>
void PairGaussElecKokkos<DeviceType>::compute(int eflag_in, int vflag_in) {
  ...
  EV_FLOAT ev = pair_compute<PairGaussElecKokkos<DeviceType>,void>(this,(NeighListKokkos<DeviceType>*)list);
...
F_FLOAT PairGaussElecKokkos<DeviceType>::
compute_fpair(const F_FLOAT &rsq, const int &, const int &, const int &itype, const int &jtype) const {
  ...
  const F_FLOAT gauss = amp * exp(-r * r/(2 * std_dev * std_dev));
  const F_FLOAT fpair = (r/std_dev * std_dev) * gauss; 
...
/* ---------------------------------------------------------------------- */
template<class DeviceType>
void PairGaussElecKokkos<DeviceType>::allocate() {
  PairGaussElec::allocate();  // Call base class allocation
  ...
  k_params = Kokkos::DualView<params_gauss**, Kokkos::LayoutRight, DeviceType>("PairGaussElec::params", n+1, n+1);
...
/* ---------------------------------------------------------------------- */
namespace LAMMPS_NS {
template class PairGaussElecKokkos<LMPDeviceType>;
#ifdef LMP_KOKKOS_GPU
template class PairGaussElecKokkos<LMPHostType>;
#endif
}
