...
total_atoms = cmd.count_atoms('dna')

# Calculate the number of repeats
num_repeats = total_atoms // 23

# Draw the chains by bonding the specified DNA particles for each repeat
for repeat in range(num_repeats):
    offset = repeat * 23
    for start, end in dna_bonds:
        cmd.bond(select_particle(start + offset), select_particle(end + offset))
...
# Adjust indices for repeats
histone_indices_repeats = [i + repeat * 23 for repeat in range(num_repeats) for i in histone_indices]
dna_indices_repeats = [i + repeat * 23 for repeat in range(num_repeats) for i in dna_indices]

# Create selections for histones and DNA particles
cmd.select('histones', ' or '.join([select_particle(i) for i in histone_indices_repeats]))
cmd.select('dna_particles', ' or '.join([select_particle(i) for i in dna_indices_repeats]))
...
for i in histone_indices_repeats:
    cmd.alter(f'dna and index {i}', 'vdw=16.5')
for i in dna_indices_repeats:
