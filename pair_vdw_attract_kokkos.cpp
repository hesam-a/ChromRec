...
#include "pair_vdw_attract_kokkos.h"
...
PairVDWAttractKokkos<DeviceType>::PairVDWAttractKokkos(LAMMPS *lmp) : PairVDWAttract(lmp) {
...
/* ---------------------------------------------------------------------- */
template<class DeviceType>
PairVDWAttractKokkos<DeviceType>::~PairVDWAttractKokkos() {
...
/* ---------------------------------------------------------------------- */
template<class DeviceType>
void PairVDWAttractKokkos<DeviceType>::settings(int narg, char **arg) {
  if (narg > 2) error->all(FLERR,"Illegal pair_style command");

  PairVDWAttract::settings(1,arg);
}

/* ---------------------------------------------------------------------- */
template<class DeviceType>
void PairVDWAttractKokkos<DeviceType>::init_style()

{
  // call the base class, which does neighbor->add_request(this,...)
  PairVDWAttract::init_style();
...
/* ---------------------------------------------------------------------- */
template<class DeviceType>
double PairVDWAttractKokkos<DeviceType>::init_one(int i, int j) {
  double cutone = PairVDWAttract::init_one(i, j);
  ...
  //printf("Init pair_coeff: i=%d, j=%d, amp=%f, std_dev=%f,  R=%f, offset=%f\n",
  //     i, j, amp[i][j], std_dev[i][j], R[i][j], offset[i][j]);
...
/* ---------------------------------------------------------------------- */
template<class DeviceType>
void PairVDWAttractKokkos<DeviceType>::compute(int eflag_in, int vflag_in) {
  ...
  EV_FLOAT ev = pair_compute<PairVDWAttractKokkos<DeviceType>,void>(this,(NeighListKokkos<DeviceType>*)list);
...
F_FLOAT PairVDWAttractKokkos<DeviceType>::
compute_fpair(const F_FLOAT &rsq, const int &, const int &, const int &itype, const int &jtype) const {
  ...
  const F_FLOAT delta_r = r - R;
  const F_FLOAT vdW = -amp * exp(-delta_r * delta_r/(2 * std_dev * std_dev));
  const F_FLOAT fpair = (delta_r/std_dev * std_dev) * vdW; 

//  printf("compute_fpair: rsq=%f, itype=%d, jtype=%d\n", rsq, itype, jtype);
//  printf("Params: amp=%f, std_dev=%f, R=%f\n",
...
//         STACKPARAMS ? m_params[itype][jtype].R : params(itype, jtype).R);
  ...
  return -amp * exp(-delta_r * delta_r/(2 * std_dev * std_dev));
}


/* ---------------------------------------------------------------------- */
template<class DeviceType>
void PairVDWAttractKokkos<DeviceType>::allocate() {
  PairVDWAttract::allocate();  // Call base class allocation
  ...
  k_params = Kokkos::DualView<params_attract**, Kokkos::LayoutRight, DeviceType>("PairVDWAttract::params", n+1, n+1);
...
/* ---------------------------------------------------------------------- */
namespace LAMMPS_NS {
template class PairVDWAttractKokkos<LMPDeviceType>;
#ifdef LMP_KOKKOS_GPU
template class PairVDWAttractKokkos<LMPHostType>;
#endif
}
